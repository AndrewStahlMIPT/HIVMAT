...
To support this need, gMock gives you the `SafeMatcherCast<RealType>(m)` function. It
casts a matcher `m` to type `Matcher<RealType>`. To ensure safety, gMock checks that
(let `U` be the type `m` accepts :

1.  Type `RealType` can be *implicitly* cast to type `U`;
2.  When both `RealType` and `U` are built-in arithmetic types (`bool`, integers, and
    floating-point numbers), the conversion from `RealType` to `U` is not lossy (in
    other words, any value representable by `RealType` can also be represented by `U`);
    and
3.  When `U` is a reference, `RealType` must also be a reference (as the underlying
...
If you find `SafeMatcherCast<RealType>(m)` too limiting, you can use a similar function
`MatcherCast<RealType>(m)`. The difference is that `MatcherCast` works as long as you
can `static_cast` type `RealType` to type `U`.
...
`DefaultValue<RealType>::Set()` (described [here](#DefaultValue)) or `ON_CALL()`.
...
one object to another, but cannot be copied. `std::unique_ptr<RealType>` is probably
...
Also, remember that `DefaultValue<RealType>` is a global resource that potentially
...
A matcher of argument type `RealType` implements the matcher interface for `RealType` and does
two things: it tests whether a value of type `RealType` matches the matcher, and can
describe what kind of values it matches. The latter ability is used for
generating readable error messages when expectations are violated.

A matcher of `RealType` must declare a typedef like:
...
// where `value` is of type `RealType` and
...
class NotNullMatcher {
 public:
  ...
  bool MatchAndExplain(RealType* p, std::ostream*) const {
    return p != nullptr;
  }
...
type `RealType` using the legacy API you have to derive from `MatcherInterface<RealType>` and
...
class MatchResultListener {
 public:
  ...
  // is NULL.
  template <typename RealType>
  MatchResultListener& operator<<(const RealType& x);
...
class MatcherInterface {
 public:
  ...
  // result to 'listener'.
  virtual bool MatchAndExplain(RealType x, MatchResultListener* listener) const = 0;
...
class NotNullMatcher {
 public:
  ...
  // a method template, or even overload it.
  template <typename RealType>
  bool MatchAndExplain(RealType* p,
...
struct MultiplyBy {
  template <typename RealType>
  RealType operator()(RealType arg) { return arg * multiplier; }
...
```cpp
// DuplicateArg<k, RealType>(output) converts the k-th argument of the mock
// function to type RealType and copies it to *output.
ACTION_TEMPLATE(DuplicateArg,
                // Note the comma between int and k:
                HAS_2_TEMPLATE_PARAMS(int, k, typename, RealType),
                AND_1_VALUE_PARAMS(output)) {
  *output = RealType(std::get<k>(args));
}
```
