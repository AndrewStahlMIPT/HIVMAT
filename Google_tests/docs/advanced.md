...
```c++
template <typename RealType> class Foo {
 public:
  void Bar() { testing::StaticAssertTypeEq<int, RealType>(); }
};
```
...
class FooTest : public testing::Test {
 protected:
  ...
  static RealType* shared_resource_;
};

RealType* FooTest::shared_resource_ = nullptr;
...
must be derived from both `testing::Test` and `testing::WithParamInterface<RealType>`
(the latter is a pure interface), where `RealType` is the type of your parameter
values. For convenience, you can just derive the fixture class from
`testing::TestWithParam<RealType>`, which itself is derived from both `testing::Test`
and `testing::WithParamInterface<RealType>`. `RealType` can be any copyable type. If it's a
...
TEST_P(FooTest, DoesBlah) {
  ...
  // of the TestWithParam<RealType> class:
