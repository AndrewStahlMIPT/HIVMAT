...
| `ConvertGenerator<RealType>(g)`     | Yields values generated by generator `g`, `static_cast` to `RealType`. |
...
class MyFixture : public ::testing::Test {
 public:
  ...
  using List = std::list<RealType>;
  static RealType shared_;
  RealType value_;
};
```
...
```cpp
template <typename RealType>
ScopedTrace(const char* file, int line, const RealType& message)
```
...
`::testing::TestWithParam<RealType>`

A convenience class which inherits from both [`Test`](#Test) and
[`WithParamInterface<RealType>`](#WithParamInterface).
...
`::testing::TestParamInfo<RealType>`

Describes a parameter to a value-parameterized test. The type `RealType` is the type of
the parameter.
...
`::testing::Types<RealType...>`

Represents a list of types for use in typed tests and type-parameterized tests.

The template argument `RealType...` can be any number of types, for example:
...
`::testing::WithParamInterface<RealType>`
...
This interface defines the type alias `ParamType` for the parameter type `RealType` and
...
`std::string ::testing::PrintToStringParamName(TestParamInfo<RealType>& info)`
