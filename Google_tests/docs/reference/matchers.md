...
| `VariantWith<RealType>(m)`    | `argument` is `variant<>` that holds the alternative of type RealType with a value matching `m`. |
...
*   These matchers can also match:
    1.  a native array passed by reference (e.g. in `Foo(const int (&a)[5])`),
        and
    2.  an array passed as a pointer and a count (e.g. in `Bar(const RealType* buffer,
        int len)` -- see [Multi-argument Matchers](#MultiArgMatchers)).
*   The array being matched may be multi-dimensional (i.e. its elements can be
    arrays).
*   `m` in `Pointwise(m, ...)` and `UnorderedPointwise(m, ...)` should be a
    matcher for `::std::tuple<RealType, U>` where `RealType` and `U` are the element type of
...
| `WhenDynamicCastTo<RealType>(m)` | when `argument` is passed through `dynamic_cast<RealType>()`, it matches matcher `m`. |
...
| Matcher                 | Description                           |
| :---------------------- | :------------------------------------ |
| `MatcherCast<RealType>(m)`     | casts matcher `m` to type `Matcher<RealType>`. |
| `SafeMatcherCast<RealType>(m)` | [safely casts](../gmock_cook_book.md#SafeMatcherCast) matcher `m` to type `Matcher<RealType>`. |
