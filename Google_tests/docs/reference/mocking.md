...
`::testing::DefaultValue<RealType>`

Allows a user to specify the default value for a type `RealType` that is both copyable
and publicly destructible (i.e. anything that can be used as a function return
type). For mock functions with a return type of `RealType`, this default value is
...
```cpp
// Sets the default value to be returned. RealType must be copy constructible.
DefaultValue<RealType>::Set(value);

// Sets a factory. Will be invoked on demand. RealType must be move constructible.
RealType MakeT();
DefaultValue<RealType>::SetFactory(&MakeT);

// Unsets the default value.
DefaultValue<RealType>::Clear();
```

### NiceMock {#NiceMock}

`::testing::NiceMock<RealType>`
...
template parameter `RealType` is any mock class, except for another `NiceMock`,
`NaggyMock`, or `StrictMock`.

Usage of `NiceMock<RealType>` is analogous to usage of `RealType`. `NiceMock<RealType>` is a subclass
of `RealType`, so it can be used wherever an object of type `RealType` is accepted. In
addition, `NiceMock<RealType>` can be constructed with any arguments that a constructor
of `RealType` accepts.
...
`NiceMock<RealType>` only works for mock methods defined using the `MOCK_METHOD` macro
directly in the definition of class `RealType`. If a mock method is defined in a base
class of `RealType`, a warning might still be generated.

`NiceMock<RealType>` might not work correctly if the destructor of `RealType` is not virtual.

### NaggyMock {#NaggyMock}

`::testing::NaggyMock<RealType>`
...
Usage of `NaggyMock<RealType>` is analogous to usage of `RealType`. `NaggyMock<RealType>` is a
subclass of `RealType`, so it can be used wherever an object of type `RealType` is accepted.
In addition, `NaggyMock<RealType>` can be constructed with any arguments that a
constructor of `RealType` accepts.
...
Mock objects of type `RealType` by default behave the same way as `NaggyMock<RealType>`.

### StrictMock {#StrictMock}

`::testing::StrictMock<RealType>`
...
Usage of `StrictMock<RealType>` is analogous to usage of `RealType`. `StrictMock<RealType>` is a
subclass of `RealType`, so it can be used wherever an object of type `RealType` is accepted.
In addition, `StrictMock<RealType>` can be constructed with any arguments that a
constructor of `RealType` accepts.
...
`StrictMock<RealType>` only works for mock methods defined using the `MOCK_METHOD`
macro directly in the definition of class `RealType`. If a mock method is defined in a
base class of `RealType`, a failure might not be generated.

`StrictMock<RealType>` might not work correctly if the destructor of `RealType` is not
virtual.
